...
pub(crate) trait FunctionScope {
    ...
    fn _prepend_var_decls<Iter: Iterator<Item = (VarDeclKind, Ident, Option<Box<Expr>>)>>(&mut self, _var_decls: Iter) {
    }
}
...
pub(crate) fn prepend_var_decls_into_stmts<Iter: Iterator<Item = (VarDeclKind, Ident, Option<Box<Expr>>)>>(
    stmts: &mut Vec<Stmt>,
    var_decls: Iter,
) {
...
pub(crate) fn prepend_var_decls_into_module_items<Iter: Iterator<Item = (VarDeclKind, Ident, Option<Box<Expr>>)>>(
...
pub(crate) fn prepend_var_decls_into_block_stmt<Iter: Iterator<Item = (VarDeclKind, Ident, Option<Box<Expr>>)>>(
    block: &mut BlockStmt,
    var_decls: Iter,
) {
...
impl FunctionScope for ArrowExpr {
    fn _get_params(&self) -> Option<Vec<Param>> {
        ...
        for pat in self.params.iter() {
            result.push(Param { span: DUMMY_SP, decorators: vec![], pat: pat.clone() })
        }
        Some(result)
    }
    fn _prepend_var_decls<Iter: Iterator<Item = (VarDeclKind, Ident, Option<Box<Expr>>)>>(&mut self, var_decls: Iter) {
...
pub(crate) fn create_key_value_prop_box(key: String, value: Box<Expr>) -> Prop {
    Prop::KeyValue(KeyValueProp { key: PropName::Str(Str::from(key)), value })
}

pub(crate) fn create_void_zero_expr() -> Expr {
    Expr::Unary(UnaryExpr { span: DUMMY_SP, op: UnaryOp::Void, arg: Box::new(Expr::Lit(Lit::Num(Number::from(0)))) })
}
...
pub(crate) fn find_decl_ident_from_decl0(decl: &Decl, idents: &mut Vec<Ident>, fn_scope_idents: &mut Vec<Ident>) {
    match decl {
        Decl::Class(decl) => {
        ...
        Decl::Var(decls) => {
            let idents = if let VarDeclKind::Var = decls.kind { fn_scope_idents } else { idents };
...
pub(crate) fn find_decl_ident_from_stmt0(stmt: &Stmt, idents: &mut Vec<Ident>, fn_scope_idents: &mut Vec<Ident>) {
    match stmt {
        Stmt::Decl(decl) => {
...
pub(crate) fn find_decl_ident_from_module_items(items: &Vec<ModuleItem>) -> (Vec<Ident>, Vec<Ident>) {
    let mut idents = Vec::new();
    let mut fn_scope_idents = Vec::new();
...
impl AstSpanAccessor for JSXMemberExpr {
    fn get_span(&self) -> Span {
        ...
        Span { lo: self.prop.get_span().lo, ..obj_span }
    }
...
impl AstSpanAccessor for JSXNamespacedName {
    fn get_span(&self) -> Span {
        ...
        Span { lo: self.ns.get_span().lo, ..name_span }
    }
